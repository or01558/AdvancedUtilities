...
delete json.result;
delete json.matches;
reslove({username:username,id:json.id,exists:json.exists,status:200,message:"Success!"})
}).catch(err => {
if(err) reslove({status:404,error:"User Not Found",exists:false});
})
})
return await status;
}
...
if(user.exists){
reslove({status:404,error:"User stats are private",exists:true})
}else{
console.log(user)
reslove({status:404,error:"User Not Found",exists:false});
}
})
})
return await status;
