...
if(err) ok({status:404,error:"Can't get the server info",message:err.message});
else ok({info:Body,status:200,message:"Success!"});
})
})
...
if(!err) Body = json.parseJs(body);
if(err) ok({status:404,error:"Can't get the server info",message:err.message});
else ok({list:Body,status:200,message:"Success!"});
})
})
